...
function StartSelector({ selected, setSelected }: { selected: string; setSelected: (value: string) => void }) {
  ...
  const handleSelect = (text: HandleSelectParams['text']): void => {
  ...
  return (
    <BlurView style={styles.container} tint="extraLight">
      <Animated.View
        style={[
          styles.selectedView,
          {
      ...
      <TouchableOpacity style={styles.textContainer} onPress={() => handleSelect('Register')}>
        <Typo size={16} numberOfLines={1} style={{ fontWeight: '500' }}>
          Register
        </Typo>
      </TouchableOpacity>
