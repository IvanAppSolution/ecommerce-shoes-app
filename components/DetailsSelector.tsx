...
function DetailsSelector({ selected, setSelected }: { selected: string; setSelected: (value: string) => void }) {
  const animatedValue = new Animated.Value(0);
  const [startRange, setStartRange] = useState(0);
  const [endRange, setEndRange] = useState(0);

  interface HandleSelectParams {
    text: 'Description' | 'Specifications' | 'Reviews';
  }

  const handleSelect = async ({ text }: HandleSelectParams): Promise<void> => {
    const range =
      text === 'Description'
        ? 0
        : text === 'Specifications'
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <TouchableOpacity style={styles.textContainer} onPress={() => handleSelect({ text: 'Description' })}>
        <Typo
          size={12}
          style={{
            fontWeight: '500',
      ...
      <TouchableOpacity style={styles.textContainer} onPress={() => handleSelect({ text: 'Specifications' })}>
        <Typo
          size={12}
          style={{
            fontWeight: '500',
      ...
      <TouchableOpacity style={styles.textContainer} onPress={() => handleSelect({ text: 'Reviews' })}>
        <Typo
          size={12}
          style={{
            fontWeight: '500',
