...
type NewBottomTabProps = {
  route: BottomTabScreenProps<TabParamList>['route'];
...
const NewBottomTab = ({ state, navigation }: NewBottomTabProps) => {
  ...
  const handleSelect = ({ routeName }: HandleSelectParams): void => {
    const range =
      routeName === 'Home'
        ? 0
        : routeName === 'Cart'
    ...
    else navigation.navigate(routeName as keyof TabParamList);
  };
